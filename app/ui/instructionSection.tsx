...
export default function InstructionSection(){
    ...
    return (
        <section className="flex flex-col items-center bg-slate-800 py-14 gap-y-14 px-6">
            <h1 className="text-white text-4xl">How it Works</h1>
            <div className="flex flex-col items-center py-3 pl-8 gap-y-8 border-slate-400 border-[1px] w-full">
                <div className="grid grid-cols-3 w-fit gap-20 text-2xl text-gray-500">
                    <button className={page===0?"text-white":"hover:text-gray-400"} onClick={() => pageHandler(0)}>1. Code</button>
                    <button className={page===1?"text-white":"hover:text-gray-400"} onClick={() => pageHandler(1)}>2. Publish</button>
                    <button className={page===2?"text-white":"hover:text-gray-400"} onClick={() => pageHandler(2)}>3. Diagram</button>
                </div>
                <div className="grid relative grid-cols-[1fr_2fr] w-full h-full">
                    <div className="flex flex-col justify-self-center gap-y-6 mt-3 mb-6">
                        <p className="text-2xl">{content[page].title}</p>
                        <p className="text-sm">{content[page].desc}</p>
                        <p className="text-sm font-bold">{content[page].bold}</p>
                    </div>
                    <Image 
                        className="place-self-center justify-self-center mb-6 w-5/6"
                        src={content[page].url}
                        alt={"1. Code"}
                        width={1000}
                        height={1000}
                    />
                    <div className="absolute bottom-8 left-2/3 -translate-x-1/2 bg-slate-600 text-white opacity-75 text-xs px-3 py-1 rounded-lg">
