...
export default function QuestionsSection() {
    ...
    return (
        <>
            <div className={`relative flex flex-col justify-center items-center w-full bg-cyan-800 overflow-hidden transition-all ${toggleSlider?"h-[7rem]":"h-[3rem]"}`}>
                <p className={`text-center hover:underline cursor-pointer transition-opacity duration-500 ${toggleSlider?"opacity-0":"opacity-100"}`} 
                onClick={()=>setToggleSlider(true)}>Need help persuading your manager?</p>
                <div className={`absolute flex flex-row justify-between items-center w-full pl-6 pr-8 py-5 transition-opacity duration-1000 ${toggleSlider?"opacity-100":"opacity-0 invisible"}`}>
                    <div>
                        <p className="text-2xl mb-2">Cut the red tape.</p>
            ...
            <div className="flex flex-col items-center py-12 px-6 bg-white">
                <h1 className="text-4xl text-black">Still have questions?</h1>
                <p className="text-sm text-gray-500 mt-5">Check out our FAQs or ping us directly in our community Slack</p>
                <div className="grid grid-cols-2 w-fit gap-x-12 justify-items-center place-items-center mt-8">
                    <Button content="View the FAQ →"/>
                    <div className="flex flex-row w-fit gap-x-2 justify-center items-center p-3 cursor-pointer">
                        ...
                        <p className="text-black">Join us on Slack</p>
                    </div>
                </div>
            </div>
        </>
